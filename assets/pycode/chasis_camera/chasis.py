...
g_Image_Grabbing_Timer=3600  # unit : second
g_isStop=0
counter = 0
# 取流回调函数Ex
...
def onGetFrameEx(frame, userInfo):
    ...
    # --- end if ---
    global counter
    print(counter, counter%4==0)
    if counter==4:
        counter = 0
    if counter%4==0:
        cv2.imwrite(r"C:\\Program Files\\HuarayTech\\MV Viewer\\Development\\Samples\\Python\\uvss_last\\ChassisCam\\" + str(time.time())+'.jpg', cvImage)
        counter==0
    counter+=1
    gc.collect()
    #cv2.imshow("w", cvImage)
    #cv2.waitKey(1)
